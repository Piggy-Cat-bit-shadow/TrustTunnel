...
| `max_auth_failures`                                   | Integer | `5`           | Failed auth attempts per IP before blocking. `0` to disable          |
| `auth_failure_refill_secs`                            | Integer | `10`          | Seconds between restoring one failed-attempt token per IP            |
| `allow_multiple_credentials_per_connection`           | Boolean | `false`       | Allow different credentials per request within one connection        |
